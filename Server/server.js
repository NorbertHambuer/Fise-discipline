...
mongo.connect(url, function (err, db) {
    ...
    let materii = dbo.collection('materii');
    let serii = dbo.collection('serii');
        ...
        if (element != "") {
            ...
            try {
                if (jsonObject.an != last_an) {
                    id_serie++;
                    serii.insert({ _id: id_serie, specializare: 'cal', an_start: an_start, an_stop: an_stop });
                    let query = {
                        an_start: an_start,
                        an_stop: an_stop
                    }
